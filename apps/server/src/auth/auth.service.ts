...
import { TokenType, OTPSessionType } from '../app.types';
...
export class AuthService {
  ...
  private async verifyUser({ otp, email }: { otp: string; email: string }) {
    try {
      ...
      const user = await this.sharedService.existAuth({ email });
  ...
  async login({ email, password }: { email: string; password: string }) {
    try {
      ...
      const userExist = (await this.sharedService.existUser({ email })) != null;
      return {
        status: 200,
  ...
  async verify({ otp, email }: { otp: string; email: string }) {
    const auth = await this.sharedService.existUser({ email });
    if (!auth || auth == null)
